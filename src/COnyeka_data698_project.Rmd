...
xgb_preprocess <- function(df, mode){
 # xgboost only accepts numeric values, so convert the categorical variable to dummies
...
```{r preprocess-data}
df = data_preprocessing(vehicle_loan_default_train_cleaned, mode = "train")
```

<h5>Develop function to train the data </h5> 
 The function `data_training` trains a machine learning model according to the mode selected (logistic, rf, xgboost, or nnet).

```{r model-training}
data_training <- function(df, mode = "logistic"){
          ...
          if(mode == "logistic"){
              ...
              df_dmatrix <- xgb_preprocess(df, mode)
              ...
              df_nnet <- xgb_preprocess(df, mode)
...
```{r logistic-model}
logistic_model <- data_training(df, mode = "logistic")
```
...
rf_model <- data_training(df, mode = "rf")
```
...
```{r xgb-model}
xgb_model <- data_training(df, mode = "xgboost")
```
...
<h5> 5.2.2.4 Model Training - Artificial Neural Networks </h5>
Train a Neural Network model for the data
```{r nnet-model}
nnet_model <- data_training(df, mode = "nnet")
```
