...
data_preprocess_scaling <- function(df){
                      ...
                      df_char <- df %>% select(loan_default, employment_type, perform_cns_score_description) %>%
                                        mutate(loan_default = factor(loan_default))
...
data_training <- function(df, mode = "logistic"){
          ...
          if(mode == "logistic"){
              ...
              rf_model <- randomForest(loan_default ~ . , data = df, ntree = 100, proximity = TRUE)
              return(rf_model)         
          ...
          return(df)
              ...
              xgb_model <- xgboost(data = df, label = df$loan_default, nthread = 2, nrounds = 2,
                                   max.depth = 4, eta = 0.2, objective = "binary:logistic")
              print("XGBoost Training complete")
              return(xgb_model)
          
        } else if(mode == "svm"){
              print("Training an SVM  Classification Model...")
              svm_model <- svm(loan_default ~ . , data = df, kernel = "linear",type = "C-classification")
...
```{r}
model1 <- data_training(df, mode = "svm")
```
...
```{r}
#loan_default_ovun_grouped <- df %>% group_by(loan_default) %>% 
                        #summarise(count = n()) %>%
                        #mutate(percentage = round((count / sum(count) * 100), 2))

#loan_default_ovun_grouped_displayed <- kable(head(loan_default_ovun_grouped, 200), "html") %>% 
                                  #kable_paper("hover", full_width = F)
#loan_default_ovun_grouped_displayed
```
