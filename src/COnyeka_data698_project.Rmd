      ...
      if(is_tibble(df) | is.data.frame(df)){
        if(mode == "test"){
          ...
          df_ovun <- ovun.sample(formula = formula(loan_default ~ .), data = get("df", sys.frame(curr_frame)),
                                 p = 0.5, seed = 1994, method = "both")$data %>% as.data.frame() %>% as_tibble()
...
model_training <- function(df, mode = "logistic"){
          ...
          if(mode == "logistic"){
              ...
              xgb_model <- xgboost(data = df_dmatrix, nthread = 4, nrounds = 50,
                                   max.depth = 10, eta = 0.1, objective = "binary:logistic")
...
model_prediction = function(df, trained_model, model_type){
  ...
  # make predictions
  if(model_type == "rf"){
    predictions = predict(trained_model, data = test_data)$predictions
  ...
  predicted = ifelse(predictions > 0.48, 1, 0)
...
model_metrics = function(actual, predicted){
  ...
  # precision
  precision_model = round((Metrics::precision(predicted, actual)), 4)
  # recall 
  recall_model = round((Metrics::recall(predicted, actual)), 4)
  # auc
...
```{r combine-results}
results = rbind(logistic_model_metrics, xgb_model_metrics, nnet_model_metrics)
kable(results, "html") %>%
                        kable_paper("hover", full_width = F) %>%
