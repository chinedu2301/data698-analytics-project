...
<h4> 5.2.2 Model Training and Testing </h4>
...
```{r}
# Train Metrics
logistic_train_actual = df_train$loan_default
logistic_train_predicted = model_prediction(df_train, logistic_model, "logistic")
logistic_train_model_metrics = model_metrics(logistic_train_actual, logistic_train_predicted)
```
```{r}
logistic_train_model_metrics
```
...
```{r clean-test-data}
# add the loan_default column to the test data so that it can be cleaned and preprocessed. drop the column later
test_data_raw <- vehicle_loan_default_test_raw %>% mutate(loan_default = 0)
# Clean the test data
cleaned_test_data = data_cleaning(test_data_raw)
# Preprocess the test data
preprocessed_test_data = data_preprocessing(cleaned_test_data, mode = "test")
```
```{r}
head(preprocessed_test_data)
```
